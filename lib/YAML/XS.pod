...
=head1 SYNOPSIS
    ...
    # Object oriented interface supporting YAML 1.2
    my $xs = YAML::XS->new;
    my $yaml = $xs->dump_string([ 1..4 ]);
    my $array = $xs->load_string($yaml);
...
The new object oriented interface is described below: L<"OBJECT ORIENTED INTERFACE">
...
This provides an interface where you can create a YAML::XS object with
options (instead of the old interface with global variables).

=item YAML 1.2 support

Also it implements the YAML 1.2 Core Schema, while the old interface
implements its own schema which is not compatible with any other module.
This is about resolving scalar values like C<null>, C<true>, C<23> as
C<undef>, boolean, number.

* YAML 1.1 / 1.2 definitions: L<https://perlpunk.github.io/yaml-test-schema/schemas.html>
* Test data: L<https://perlpunk.github.io/yaml-test-schema/data.html>
...
If set to 1, the output from dumping will be utf8 encoded.
...
=item load_string
    ...
    my $array = $xs->load_string($yaml);
    # [23]
    my @documents = $xs->load_string($yaml);
    # [
    #   [23],
    #   { some => "mapping" }
    # ]
...
=item dump_string
    ...
    my $yaml = $xs->dump_string($data);
    my $yaml = $xs->dump_string($data1, $data2, $data3);
...
=head1 OOP INTERFACE (experimental)
...
=item new
  ...
  my $xs = YAML::XS->new( indent => 8, utf8 => 1);
...
The amount of indentation spaces when dumping

=item utf8

Default: false

When false, the loader will accept utf8 decoded strings, and the dumper returns
utf8 decoded strings.

When true, the loader accepts utf8 encoded strings, and the dumper returns
utf8 encoded strings. This typically makes sense when you read from / write
to a file directly.
...
A YAML file or string can have more than one document.

In scalar context, it only loads the first document. In list context,
it returns all.

=item dump_string

    $yaml = $xs->dump_string($data);
    $yaml = $xs->dump_string($doc1, $doc2);
    $yaml = $xs->dump_string(@data);

It takes one or more parameters and returns a string with one or multiple
YAML documents.

=back

=head2 Differences to the functional interface

=over

=item YAML 1.2 Core Schema

The OOP Interface implements the 1.2 Core Schema by default: L<https://perlpunk.github.io/yaml-test-schema/schemas.html>

This way it's compatible to YAML::PP and YAML processors in other languages
supporting YAML 1.2.

The functional interface only recognizes the following special values:
C<true>, C<false>, C<null>, C<~> and anything that C<looks_like_number()>.

=item utf8

By default it will accept and return utf8 decoded strings, unlike the
